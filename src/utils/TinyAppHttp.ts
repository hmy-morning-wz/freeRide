...
const SECURE_CODE ="3d6c0ef672d6fe454797f282b14440e4654a44f280c8dfbb9b7aa074b9994782"
...
function HMAC256Sign(obj:any,key:string){
   let list:string[] = []
   ...
   let keys = Object.keys(obj).sort()
     ...
     { 
         if(key!='sign')  list.push(`${key}=${obj[key]}`) 
     }
    })    
    if(list.length) {   
   let msg = list.join("&")
   //console.log(msg)
   let sign =_.arrayBufferToBase64(HMAC256(msg,key,{asBytes:true}))
   obj.sign = sign
    }
   return obj
}
...
function _request_(httpConfig: TinyAppHttpConfig, requsetConfig: TinyAppHttpRequset) {
    ...
    if(isObject(data)&& httpConfig.appscrete) { 
        data = HMAC256Sign(data,httpConfig.appscrete)
    }
...
let  g_traceSn = 0
let g_traceHashCode:string
function  genTraceId({userId,appId}:any) {
    let sn = g_traceSn || 0
    g_traceSn = sn+1
    let traceHashCode: string
    if ((!g_traceHashCode) && userId && appId) {
      if (+appId && +userId) {
        g_traceHashCode = (+appId + +userId).toString(16)
      }
      else {
       g_traceHashCode =  _.hashCode(appId).toString(16) + _.hashCode(userId ).toString(16)
      }
    }
    traceHashCode = g_traceHashCode || _.hashCode(appId +userId).toString(16)
    traceHashCode =  traceHashCode + (+Date.now()-1600000000000).toString(16) + (Math.floor(256 * Math.random())).toString(16) + sn.toString(16)
    traceHashCode =_.arrayBufferToBase64(_.hexToArray(traceHashCode)).replace(/=|\+|\/|$/g, "")
...
function getDefaultHeader(data: TinyAppHttpConfig) {   
    ...
    let expTime = timestamp+180000
    let traceId
    let secureCode
    try{
       traceId= genTraceId({appId:APP_ID,userId:data.userId|| getApp().globalData.userId})
       secureCode=data.secureCode && _.arrayBufferToBase64( HMAC256(APP_ID+data.access_token+expTime+traceId,data.secureCode,{asBytes:true}))
    }catch(err){
      console.error(err)
    }
    let res: any = {
